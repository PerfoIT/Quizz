...
export async function listQuizzes(userId: string): Promise<PublicQuiz[]> {
  const quizzes = await prisma.quiz.findMany({
    where: { OR: [{ ownerId: userId }, { visibility: "ORGANIZATION" }] },
...
export async function ensureCanControlSession(code: string, userId: string) {
  const session = await prisma.session.findUnique({ where: { code }, select: { hostId: true } });
  if (!session) throw new Error("Session introuvable.");
  if (session.hostId && session.hostId !== userId) throw new Error("Vous ne pouvez pas piloter cette session.");
}
